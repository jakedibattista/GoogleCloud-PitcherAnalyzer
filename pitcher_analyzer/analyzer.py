...
class PitcherAnalyzer:
    ...
    def _analyze_with_gemini(self, video_path, pitcher_name, pitch_type, game_type):
        ...
        try:
            ...
            # Read the video file
            logger.info(f"Reading video file from: {video_path}")
            with open(video_path, "rb") as f:
                video_bytes = f.read()
            logger.info(f"Successfully read {len(video_bytes)} bytes from video file")
            
            # Create the prompt
            prompt = """
            Please analyze this baseball pitching video and describe:
            1. What you observe in the pitcher's motion
            2. Any notable mechanics or technique elements
            3. Areas that stand out (both positive and negative)
            
            Be specific and detailed in your observations.
            """
            
            logger.info("Sending request to Gemini API with video...")
            try:
                # First, try to generate content with just the video
                ...
                # If we got a response, create a structured result
                analysis_text = response.text.strip()
                
                result = {
                    'mechanics_score': 75,  # We'll keep a default score for now
                    'deviations': [
                        "Gemini's Analysis:",
                        "-------------------",
                        analysis_text
                    ...
                    'recommendations': [
                        "Based on the analysis above, consider working with a pitching coach to address any identified issues.",
                        "Record more videos from different angles to get a complete view of your mechanics.",
                        "Focus on the specific elements mentioned in the analysis for improvement."
                    ]
                }
                
                logger.info("Successfully created analysis result")
                return result
...
def check_and_create_bucket(project_id=None, bucket_name=None):
    ...
    bucket = storage_client.lookup_bucket(bucket_name or Config.GCS_BUCKET)
    
    if bucket is None:
        print(f"Bucket {bucket_name} does not exist. Creating...")
        bucket = storage_client.create_bucket(bucket_name, location="us-central1")
        print(f"Bucket {bucket_name} created in us-central1")
    else:
